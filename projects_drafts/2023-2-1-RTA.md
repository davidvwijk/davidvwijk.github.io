...
Run time assurance (RTA), is a general framework that monitors the desired control actions of a primary controller, and can intervene in real time when necessary to ensure safety of the system. In the below figure, the desired control, $$\boldsymbol{u}_{\rm des}$$, is modified if the RTA filter deems that $$\boldsymbol{u}_{\rm des}$$ would result in an unsafe state, and a safe control input, $$\boldsymbol{u}_{\rm act}$$, is passed to the plant. Also called a <i>safety filter</i>, an ideal RTA system should be <i>minimally invasive</i>, such that the safety filter only modifies the desired control input of the primary controller when absolutely necessary to ensure safety. In this way, the task of ensuring system safety, and the task of achieving performance objectives of the primary controller are decoupled. This decoupling is very useful when traditional offline verification procedures for the primary controller cannot be used, such as in the case with learning based techniques. This makes RTA an excellent candidate for development of safe RL controllers. 

<figure>
  <img src="/project_files/rta/rta-general.svg" wdith="300">
  <figcaption> <b> General feedback control system using RTA. </b> </figcaption>
</figure>

In this short summary, I will cover control barrier functions (CBF's) and an application of CBF's known as active set invariance filters (ASIF). The summary is primarily based on the following references: [1], [2].
